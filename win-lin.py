...
def open_settings():
    ...
    def confirm_menu_bar():
        with open("settings/window/title_bar.txt", "w") as title_file:
            new_value = f"{title_switch.get()}"
            title_file.write(new_value)
            CTkMessagebox(title = "Changes Applied", message = f"Restart pyText to show changes. (by using {restart_bind})")
    ...
    title_switch_value = StringVar(value = title_bar)
    title_switch = CTkSwitch(window_frame, text = "Title Bar", onvalue = "on", offvalue = "off", variable = title_switch_value, command = confirm_menu_bar)

    if platform.system() != "Windows":
        title_switch.configure(state = "disabled")
